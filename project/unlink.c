...
void my_unlink(char *path)
{
	...
	//ino is the ino of the selected link or file
	...
	//ip now points directly at the inode we want to unlink
	ip = &mip->INODE;
	...
	//Deallocate its blocks to prepare for freeing inode
	...
	//gets the parent dir_name of path
	strcpy(temp, path);
	strcpy(my_dirname, dirname(temp));

	//gets the basename of the unlink
	strcpy(temp, path);
	...
	//searches for basename in dir and frees it and then condenses memory
